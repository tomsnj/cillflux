...
- **Immich Postgres → 17.6 — DONE 2026-09-26** (commit `e4bdeab0`).
  Migrated 14.19 → 17.6 by logical dump/restore onto a new PVC;
  **4 min 6 s downtime**, all 66 tables verified identical by real
  `count(*)`, both vchordrq indexes rebuilt and serving, smart search
  confirmed working through the API. PR #960 was **closed, not
  merged** — it targeted 16, and an image bump alone can never work
  here: Postgres will not read a v14 data directory with a newer
  binary and this database is a plain Deployment, not PGO, so there is
  no `PGUpgrade` path. **Treat every future Renovate bump of this
  image the same way: it is never a merge, it is always
  `docs/plan-immich-postgres-pg17.md`.** Next forced round is PG17's
  EOL, November 2029.
  **Outstanding — Phase 5 cleanup, due on or after 2026-10-03:** the
  two rollback PVCs `immich-postgres-data-nvme` (the PG14 directory)
  and `immich-postgres-data` (gsks0, pre-NVMe) are still declared in
...
**A small fraction of Samsung-era JPEGs never get a thumbnail, and the cause is not the SEF trailer**  
Some Galaxy-camera JPEGs (`20210903_153528.jpg` naming) import fine -
Immich stores them, the original downloads and views correctly - but
thumbnail generation fails with `AssetGenerateThumbnails: Input file
contains unsupported image format` and the asset shows as a **blank
tile in the timeline**. Measured rate: **11 of 6,733** Samsung-named
assets, about 0.2%.

**The obvious suspect is wrong.** These files carry a Samsung Extended
Format trailer - a video and metadata block appended *after* the JPEG
EOI, with the last four bytes literally `SEFT` - and it is tempting to
blame it. It is not the cause: assets *with* an intact `SEFT` trailer
thumbnail perfectly well (verified by reading the stored originals of
working assets), and the trailer appears in both the working and the
failing files. The real defect in those 11 was never established.
Don't strip trailers hoping to fix it.

What this means in practice: expect roughly 0.2% blank tiles from any
Samsung-sourced batch and leave them - the originals are intact and
the photos are not lost. Don't chase it as a failed upload or a stuck
queue either; re-running `thumbnailGeneration` with the missing-only
command finds nothing to do, because nothing is queued or failing -
the job already ran and gave up.

**Separately, and genuinely not a problem: hidden Motion Photo video
components legitimately have no thumbnail.** Immich extracts the video
half of a Pixel `.MP.jpg` as its own asset with `visibility='hidden'`,
linked to its parent still via `livePhotoVideoId`; those are never
displayed on their own and are missing nothing. 864 of them against
those 11 real failures in the same import - so a thumbnail-coverage
audit that doesn't filter on `visibility='timeline'` looks roughly
